...
public class CategoryApiClient : CommonApiClient, ICategoryApiClient
{
    ...
    public async Task<ApiDataResult<CategorySummary>> UpdateProductAsync(CreateCategoryRequestBody requestBody, CancellationToken cancellationToken)
    {
        var url = $"{GetBaseUrl()}{ApiUrlConstants.UpdateCategory}";

        var result = await PutAsync<CreateCategoryRequestBody, CategorySummary>(url, requestBody, cancellationToken);
